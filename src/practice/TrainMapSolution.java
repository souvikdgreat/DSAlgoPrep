...
public class TrainMapSolution {
    ...
    private static class TrainMap {
        ...
        public List<Station> shortestPath(String from, String to) {
            ...
            HashMap<String, Visited> visited = new HashMap<>();
            stationQueue.add(fromStation);
            visited.put(fromStation.name, new Visited(null, 0));

            while (!stationQueue.isEmpty()) {
                Station station = stationQueue.poll();
                int distance = visited.get(station.name).distance;
                for (Station neighbourStation : station.neighbours) {
                    if (toStation.equals(neighbourStation)) {
                        Visited neighbourStationVisited = new Visited(station, distance + 1);
                        visited.put(neighbourStation.name, neighbourStationVisited);
                        stationQueue.clear();
                        break;
                    }
                    if (!visited.containsKey(neighbourStation.name)) {
                        Visited neighbourStationVisited = new Visited(station, distance + 1);
                        visited.put(neighbourStation.name, neighbourStationVisited);
                        stationQueue.add(neighbourStation);
                    }
                }
            }

            Station ss = toStation;
            while (ss != null) {
                result.add(0, ss);
                ss = visited.get(ss.name).parent;
            }
            return result;
        }
    ...
    public static boolean doTestsPass() {
        ...
        //trainMap.shortestPath("King's Cross St Pancras", "Angel");
